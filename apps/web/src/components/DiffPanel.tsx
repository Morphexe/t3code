...
export function PatchDiffViewer(props: {
  patch: string | undefined;
  cacheScope: string;
  cwd?: string | null;
  emptyLabel?: string;
  ...
  return (
    <div
      className={cn(
        "flex min-h-0 flex-col overflow-hidden rounded-md border border-border/60 bg-card/25",
        props.className,
      )}
    >
      <div className="flex h-10 shrink-0 items-center justify-end gap-1 border-b border-border/50 px-2">
        <ToggleGroup
          className="shrink-0"
          variant="outline"
          size="xs"
      ...
      {!renderablePatch ? (
        <div className="flex min-h-40 items-center justify-center px-3 py-2 text-xs text-muted-foreground/70">
          {props.emptyLabel ?? "No patch available."}
        </div>
